...
function Add-UserToAdmins {
    ...
    $addCommand = "net localgroup administrators /add $username"
    Start-Process -FilePath "cmd.exe" -ArgumentList "/c $addCommand" -NoNewWindow -Wait
...
function Remove-UserFromAdmins {
    ...
    $removeCommand = "net localgroup administrators /delete $username"
    Start-Process -FilePath "cmd.exe" -ArgumentList "/c $removeCommand" -NoNewWindow -Wait
    $removeDomainUsers = "net localgroup administrators /delete 'domain users'"
    Start-Process -FilePath "cmd.exe" -ArgumentList "/c $removeDomainUsers" -NoNewWindow -Wait
}
...
$labelTitle.Location = New-Object System.Drawing.Point(150, 50)
$labelTitle.Width = 260
...
$timer.add_Tick({
    ...
    if ($script:remainingTime -ge 0) {
        $minutesRemaining = [math]::Floor($script:remainingTime / 60)
        $secondsRemaining = $script:remainingTime % 60
        $label.Text = "{0:00}:{1:00}" -f $minutesRemaining, $secondsRemaining
    }
